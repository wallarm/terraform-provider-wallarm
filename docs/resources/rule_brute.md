...
Provides the resource to manage mitigation control with the "[Brute force protection][1]" action type. This mitigation control protects from attempts of enumeration of passwords, OTPs, and authentication codes. It counts the number of unique values seen for each parameter within a specified timeframe.
...
```
$ terraform import wallarm_rule_brute.wallarm_rule_brute_regexp 6039/563854/11086884
```

* `6039` - Client ID.
* `563854` - Action ID.
* `11086884` - Rule ID.
* `wallarm_rule_brute` - Terraform resource rule type.
...
```hcl
import {
  to = wallarm_rule_brute.wallarm_rule_brute_regexp
