...
Provides the resource to manage rules with the "Allow binary data" action type. Allows fine-tuning attack detection for request points containing binary data (e.g. archived or encrypted files). When analyzing the specified request point, the Wallarm node will ignore attack signs that explicitly cannot be passed in binary data.
