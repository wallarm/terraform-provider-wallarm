...
* `permissions` - (**required**) user role. Can be one of: `admin`, `admin_ext`, `analytic`, `auditor`, `deploy`, `partner_admin`, `partner_admin_ext`, `partner_analytic`, `partner_auditor`. Roles description is available in the [Wallarm official documentation](https://docs.wallarm.com/user-guides/settings/users/#user-roles).
