...
* **fix(enumerated_parameters bool fields):** `additional_parameters` and `plain_parameters` switched from `Optional+Default:false` to `Optional+Computed`. The API defaults both to `true` in regexp mode and omits them entirely in exact mode; the previous `Default:false` caused state to diverge from API value on regexp-mode imports, and made auto-generated post-import configs hit the strict-denial validator. With `Computed: true`, SDK preserves API-echoed state across plans. New mapper helper `EnumeratedParametersFromResourceData` strips unset bool keys (via `d.GetRawConfig()`) before building the API payload — the wire body omits the field when the user didn't write it, and the API's `true` default wins. The mapper itself now uses the `, ok` type-assert form so missing keys produce nil pointers (parallel to the v0.12.1 `*int` pattern for rate-limit fields). Affects `wallarm_rule_brute`, `_bola`, `_enum` Create + Update paths.
