...
* **schema(rule): dropped `Computed: true` from user-controlled fields where it caused SDKv2 zero-value updates to be silently ignored.**
  * Fields affected: `set`, `title`, `enumerated_parameters.{name_regexps, value_regexps, additional_parameters, plain_parameters}`, `reaction.{block_by_session, block_by_ip, graylist_by_ip}`.
  * `additional_parameters` and `plain_parameters` gain `Default: false`.
  * **Effect:** existing imported state with these fields populated and HCL configs that don't include them will produce a one-time clearing diff on next plan. Add the fields to your HCL with desired values, or accept the clear.
  * **Retained `Optional+Computed`:** `mitigation` (API-derived); `active` (migration risk for imports of disabled rules — tracked as follow-up); `max_alias_size_kb` (`ForceNew` + zero-value would force replace on existing tests; tracked as follow-up).
...
* **fix(rule_brute, rule_bola, rule_enum):** `enumerated_parameters` validation tightened — exact mode rejects `additional_parameters=true` / `plain_parameters=true` (literal `false` is allowed because auto-generated configs after import surface explicit `false`, and the mapper drops these fields from the wire body in exact mode regardless of value). Regexp mode requires both `name_regexps` and `value_regexps` at plan time. The TF→API mapper no longer substitutes `[""]` for empty regexp lists — that workaround round-tripped from the API as `[null]` in state and produced a perpetual diff. Migration: configs that previously omitted one of the regexp lists must add `name_regexps = [""]` or `value_regexps = [""]` explicitly to opt out of that filter.
* **fix(enumerated_parameters bool fields):** `additional_parameters` and `plain_parameters` switched from `Optional+Default:false` to `Optional+Computed`. The previous `Default:false` caused two issues: (a) when an API-side rule was imported with these fields set to `true`, auto-generated configs surfaced the explicit values and re-applies tried to flip them back to `false`; (b) post-import configs in exact mode literally contained `additional_parameters = false` from the SDK fill, which the strict-denial validator then rejected. With `Computed: true`, the SDK preserves API-echoed state across plans. The exact-mode validator was loosened to error only on `true` so that auto-generated `false` values are harmless (the mapper drops these fields from the wire body in exact mode regardless of value). The mapper now uses the `, ok` type-assert form so future callers building the block map directly can pass nil pointers; under normal Create/Update the SDK fills both keys with `false` and the wire body sends both, which is what the regexp-mode API requires.
* **fix(arbitrary_conditions point round-trip):** the API stores `arbitrary_conditions[*].point` as a flat array (e.g. `["post","json_doc","hash","user_id"]`), but Terraform's HCL expresses the same chain as 2D (`[["post"], ["json_doc"], ["hash","user_id"]]`). `ArbitraryConditionsToTF` previously wrapped the flat array as a single sub-array, so any HCL with multi-step point chains produced a force-replacement plan diff on every cycle. Now uses `WrapPointElements` to re-chunk the flat list per the paired/simple element rules, matching the HCL representation. Affects `wallarm_rule_brute`, `_bola`, `_enum`, `_forced_browsing`.
* **fix(mapper_tftoapi):** regexp-mode `name_regexps`/`value_regexps` lists with an empty-string element now reach the API. SDKv2 normalizes empty strings in TypeString lists to `cty.NullVal`, which the shared `ConvertToStringSlice` was silently dropping — leaving the JSON field absent and the API rejecting the regexp-mode payload as missing required keys. New `convertRegexpList` preserves `nil → ""` so an HCL `value_regexps = [""]` survives end-to-end.
...
* **refactor(rule):** `resourcerule.Update` accepts variadic `UpdateCustomizer`s; new `With*` helper library wires per-resource Update in one line.
...
* **build(makefile):** dropped `-race` from `make testacc` until remaining rule tests migrate to the v2.3.5 pattern (`ProtoV5ProviderFactories` + `testAccNewAPIClient`); unit tests still run under `-race`.
