...
* **Plan-time enum validators added to `wallarm_rule_regex.attack_type`, `wallarm_rule_disable_attack_type.attack_type`, `wallarm_rule_vpatch.attack_type`** — plan-time fail with the documented enum lists instead of waiting for the API to reject. No behavior change.
