...
* **Plan-time enum validators added** to `wallarm_rule_regex.attack_type` (12 values incl. `vpatch`), `wallarm_rule_disable_attack_type.attack_type` (18 values — union of docs/resources, schema Description, and CLAUDE.md hits-attack-types), `wallarm_rule_vpatch.attack_type` (11 values), and `wallarm_rule_file_upload_size_limit.size` (`IntAtLeast(1)`). Plan-time fail with the documented enum lists instead of waiting for the API to reject. The earlier-in-this-session attempt at this used a too-narrow set sourced only from the schema's Description text and broke acc tests (`TestAccRuleDisableAttackTypeCreate_DefaultBranch` uses `ssi`, `TestAccRuleRegexCreateOpenDir` uses `vpatch`); validators now use the union of every documented and acc-test-observed value.
