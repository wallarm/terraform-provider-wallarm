...
* **Plan-time enum validator added to `wallarm_rule_file_upload_size_limit.size`** (`IntAtLeast(1)`) — plan-time fail for the API's `1..2^64` range. No behavior change. *Note: attack_type validators briefly added on `wallarm_rule_regex` / `_disable_attack_type` / `_vpatch` were reverted — the API-accepted set is broader than the schema Description text and maintaining a complete allowlist is fragile (e.g. `regex.attack_type=vpatch`, `disable_attack_type=ssi` are both valid but absent from the docs).*
