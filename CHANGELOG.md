...
* **fix(rule_brute, rule_bola, rule_enum):** `enumerated_parameters` validation tightened — exact mode now rejects `additional_parameters` and `plain_parameters` whenever they appear in HCL (any value, not just `true`), via `d.GetRawConfig()` to bypass the SDK's `Default: false` masking. Regexp mode now requires both `name_regexps` and `value_regexps` at plan time. The TF→API mapper no longer substitutes `[""]` for empty regexp lists — that workaround round-tripped from the API as `[null]` in state and produced a perpetual diff. Migration: configs that previously omitted one of the regexp lists must add `name_regexps = [""]` or `value_regexps = [""]` explicitly to opt out of that filter.
