...
func TestAccIntegrationSumologicFullSettings(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testWallarmIntegrationSumologicFullConfig(rnd, "tf-test-"+rnd, "https://endpoint6.collection.us2.sumologic.com/receiver/v1/http/ZaVnC4dhaV123gN3o--AIj3q8y9GrwxSrAgcOJMvltRVnEIAIyR001VBlDsTYGBpieGxBxyJZA1eFIZcuyJ_ivkjPZ6Ynl8x3kLBJi4arZ479cD8ePJsqA=="),
				Check: resource.ComposeTestCheckFunc(
...
func testWallarmIntegrationSumologicFullConfig(resourceID, name, token string) string {
	return fmt.Sprintf(`
resource "wallarm_integration_sumologic" "%[1]s" {
	name = "%[2]s"
	sumologic_url = "%[3]s"
