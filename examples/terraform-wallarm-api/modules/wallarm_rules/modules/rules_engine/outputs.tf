output "rule_ids" {
  description = "Map of rule keys to their created rule IDs (all resource types)"
  value = merge(
    ...
    { for k, v in wallarm_rule_uploads.expanded : k => v.id },
    { for k, v in wallarm_rule_uploads.single : k => v.id },
    { for k, v in wallarm_rule_ignore_regex.this : k => v.id },
    { for k, v in wallarm_rule_parser_state.expanded : k => v.id },
    { for k, v in wallarm_rule_parser_state.single : k => v.id },
